...
__global__ void trans_dev(double* B, int N)
{
...
__global__ void mult_mat_device(double* A, double* B, double* C, int N)
{
	...
	__shared__ double shared_A[threads];

	double tmp_sum = 0;

	if (i < N && j < N)
	{
		for (int z = 0; z < N / threads; z++)
		{
			shared_A[threadIdx.x] = A[i * N + z * threads + threadIdx.x];
			__syncthreads();

			for (int k = 0; k < threads; k++)
			{
				//tmp_sum += shared_A[k] * B[j * N + z * threads + k];
				tmp_sum += shared_A[k] * B[(z * threads + k) * N + j];
			}
		}
		C[i * N + j] = tmp_sum;
	}
...
int main()
{
	...
	cudaEventRecord(event1, 0);
	//trans_dev << <blocks, threads >> > (d_B, N);
