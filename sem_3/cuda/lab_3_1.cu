	...
	for (int i = 0; i < N; i++)
		for (int j = 0; j < N; j++)
			...
			else
			{
				...
				/*
				h_B_r[i * N + j] = 1.0 / 9.0 * (h_A_r[(i - 1) * N + j - 1] + h_A_r[i * N + j - 1] + h_A_r[(i + 1) * N + j - 1]
					+ h_A_r[(i - 1) * N + j] + h_A_r[i * N + j] + h_A_r[(i + 1) * N + j] + h_A_r[(i - 1) * N + j + 1]
					+ h_A_r[i * N + j + 1] + h_A_r[(i + 1) * N + j + 1]);
				h_B_g[i * N + j] = 1.0 / 9.0 * (h_A_g[(i - 1) * N + j - 1] + h_A_g[i * N + j - 1] + h_A_g[(i + 1) * N + j - 1]
					+ h_A_g[(i - 1) * N + j] + h_A_g[i * N + j] + h_A_g[(i + 1) * N + j] + h_A_g[(i - 1) * N + j + 1]
					+ h_A_g[i * N + j + 1] + h_A_g[(i + 1) * N + j + 1]);
				h_B_b[i * N + j] = 1.0 / 9.0 * (h_A_b[(i - 1) * N + j - 1] + h_A_b[i * N + j - 1] + h_A_b[(i + 1) * N + j - 1]
					+ h_A_b[(i - 1) * N + j] + h_A_b[i * N + j] + h_A_b[(i + 1) * N + j] + h_A_b[(i - 1) * N + j + 1]
					+ h_A_b[i * N + j + 1] + h_A_b[(i + 1) * N + j + 1]);
					*/
}
