...
to setup
  ...
  create-targets targets-number [setup-target]
  color-patches

  create-places places-number [setup-place]
...
to color-patches
  ask patches [set pval eval pxcor pycor]
  let max-val [pval] of max-one-of patches [pval]
  ask patches [set pcolor my-scale-color (pval / max-val)]
end

to-report my-scale-color [x]
  set x x ^ 0.75
  report rgb 0 (225 * x) (255 - 255 * x)
end

to go
  ask targets [fd targets-vel]
  color-patches
  ...
  if foragers-number > 0 [
    ask places [
      hatch-bees foragers-number [
        set my-place myself
        set xcor xcor + ((random-float 1) - 0.5) * places-size
        set ycor ycor + ((random-float 1) - 0.5) * places-size
  ...
  if scouts-number > 0 [
    let worst max-one-of places [val]
  ...
  ask places [
    ...
    if closest != nobody
    [
    if [distance myself] of closest < places-size / 2 [
      ifelse val < [val] of closest [
        ask closest [setxy random-xcor random-ycor]
      ]
      [setxy random-xcor random-ycor]
    ]
    ]
  ...
  ask targets [get-distance]
  tick
end

to get-distance
  set p-dist [distance myself] of min-one-of places [distance myself]
end

to-report max-distance
  report [p-dist] of max-one-of targets [p-dist]
end
@#$#@#$#@
GRAPHICS-WINDOW
210
...
"" ""
PENS
"default" 1.0 0 -3844592 true "" "plot max-distance"
