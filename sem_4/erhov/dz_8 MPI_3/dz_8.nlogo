...
to setup-map
  ...
  if map-type = "blackhole" [
    ask patches with [(abs(pxcor) < 10) and (abs(pycor) < 10) ] [set free? false]
  ]
  if map-type = "bricks" [
    ask patches [
...
to setup-targets
  ...
  while [minv < 10 ^ (-7)] [
    ...
    ask patches [set p-val (p-val - minv) / (maxv - minv)]
    ask free-patches [set pcolor my-scale-color (p-val ^ 0.3)]
    display
end
...
to go
  ...
  if selection? [
    ask bacteria [
      if (random-float 1) < sel-p [contest]
    ]
  ]
...
to contest
  let rival one-of other bacteria
  let r-val [val] of rival
  ifelse val < r-val
  [
    ask rival [hatch 1]
    die
