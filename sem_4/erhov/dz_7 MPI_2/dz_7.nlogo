breed [nodes node]
breed [ants ant]
links-own [len ph visits best? local?]
globals [best-len sel rnd best-ant]
...
to go
  ...
  check-best
  let max-ph [ph] of max-one-of links [ph]
  ask links [
    set ph ph / max-ph
    color-link
  ]
...
to color-local
  if show-best? [
    ask [visited-links] of best-ant [
      if not best? [set thickness 0.15]
    ]
  ]
end
...
to check-best
  set best-ant min-one-of ants [path-len]
  let bp [path-len] of best-ant
  if ticks = 0 or bp < best-len [
    set best-len bp
    ask links [set best? false]
    ask [visited-links] of best-ant [set best? true]
  ]
  ask links [set local? false]
  ask [visited-links] of best-ant [set local? true]
end
...
layout
layout
"random" "circle" "circle-centre" "circle-2" "star" "grid"
3
