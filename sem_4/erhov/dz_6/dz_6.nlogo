...
to setup
  ...
  if setup-ph? [ ask patches [
    set ph-n random-float 0.01 + 0.01
...
to go
  ...
  ask patches [
    let n ph-n * 0.99
    let f ph-f * 0.99
    ifelse n > 0.001 [set ph-n n] [set ph-n 0]
    ifelse f > 0.001 [set ph-f f] [set ph-f 0]
  ]
  ask patches [color-patch]
  tick
end
...
to rotate-ant
  ...
  let p patches in-cone vision angle
  with [distance myself > vision / 2]
  if state = "forward" [
