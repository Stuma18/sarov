...
to-report eval-target[x y]
  ...
  if target-function = "rastrigin"[
    report x ^ 2 + y ^ 2 + 50 * (2 - cos (100 * x) - cos (100 * y))
  ]
end

to setup-landscape
  ifelse target-function != "random"[
    ask patches[
      set val eval-target pxcor pycor
    ]
  ]
  [ask patches[ set val random-float 1]
...
to go
  ...
  ask particles [
    let h heading
    if xcor != p-x or ycor != p-y [set h towardsxy p-x p-y]
    ...
    let gy g-y
    if not global?[
      let best-partner min-one-of particles in-radius vision [p-val]
      set gx [p-x] of best-partner
      set gy [p-y] of best-partner
    ]

    set vx gamma * vx + alpha * d * sin h
    set vy gamma * vy + alpha * d * cos h
    ;set h towardsxy g-x g-y
    if xcor != gx or ycor != gy [set h towardsxy gx gy]
    set d distancexy gx gy
    let beta random-float beta-max
    set vx vx + beta * d * sin h
    set vy vy + beta * d * cos h
    let v1 sqrt (vx ^ 2 + vy ^ 2)
    if v1 > v-max [set vx vx / v1 * v-max
    set vy vy / v1 * v-max]
...
"" ""
PENS
"average" 1.0 0 -2674135 true "" "plot mean [val] of particles"
"best" 1.0 0 -13345367 true "" "plot min [val] of particles"
