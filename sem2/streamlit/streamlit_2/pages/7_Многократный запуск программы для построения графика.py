...
st.set_page_config(page_title = "Многократный запуск программы для построения графика")

st.sidebar.success("Многократный запуск программы для построения графика")

st.write("# Многократный запуск программы для построения графика")

k = st.sidebar.slider('Число запусков программы:', 1, 100, 10)

eps = st.sidebar.slider('Эпсилон:', 0.1, 0.2, 0.1)

N_takt = st.sidebar.slider('Количество тактов:', 10, 1000, 30, step = 10)
...
for s in range(k):
    ...
    ax.set(xlabel='Такты', 
        ...
        title='Моделирование работы алгоритма $Eps-greedly$')
    colors = ["red",
            "black",
            "magenta",
            "blue",
    ...
    # сохраняем массив ver целиком
    all_vers.append(ver)


# в итоге all_vers - массив массивов ver
# i от 0 до длины массива ver
for i in range(len(all_vers[0])):
    sum = 0
    # j от 0 до числа массивов ver
    for j in range(len(all_vers)):
        sum += all_vers[j][i]
    avg = sum / len(all_vers)
    print(f"Average ver_{i} = {avg}")
    grafik_3.append(avg)
...
for i in range(len(all_vers[0])):
    for j in range(len(all_vers)):
        m.append(all_vers[j][i])

chunks = list(partition(k, m))

for i in range(len(chunks)):
    min_.append(min(chunks[i]))
    max_.append(max(chunks[i]))

# для графика 3
fig_3, ax_3 = plt.subplots()
ax_3.set(xlabel = 'Такты / 10', 
    ylabel = f'Зачение вероятности показа баннера $2$ \n за {k} повторений', 
    title = f'Вероятность показа баннера $2$ \n за {k} повторений')
colors = ["red", "black", "magenta", "blue", "grey", "green"]

blue_patch = mpatches.Patch(color = 'blue', label = 'Среднее значение')
green_patch = mpatches.Patch(color = 'green', label = 'Максимальное значение')
red_patch = mpatches.Patch(color = 'red', label = 'Минимальное значение')
ax_3.legend(handles = [blue_patch, green_patch, red_patch])

x_3 = np.arange(N_takt / col_takt)
ax_3.plot(x_3, grafik_3, c = "blue", linestyle = 'solid', lw = 1.5)
ax_3.plot(x_3, max_, c = "green", linestyle = 'solid', lw = 1.5)
ax_3.plot(x_3, min_, c = "red", linestyle = 'solid', lw = 1.5)
ax_3.grid(True)
ax_3.set_ylim(-0.1, 1.1)
